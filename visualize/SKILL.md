...
**Default (no command, or the first argument is a topic):** `{{command_prefix}}visualize <topic-or-path>` creates a new artifact. The agent picks a template from intent across all of `templates/`, composes against the brand's custom tokens from `DESIGN.md` + sidecar `tokens.css` (or a greenfield seed from `scripts/palette.mjs` when `DESIGN.md` is absent), writes self-contained HTML. **The flow is gated:** preflight checklist + shape-gate before code, no self-authored briefs. See `reference/create.md`.
