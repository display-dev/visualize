...
2. **Codebase sniff** *(optional, ask first)*. If the project has Tailwind / CSS-token / framework / logo signals, offer: "Want me to scan your codebase for design tokens, fonts, and logo paths?" If yes:
   ...
   ```sh
   node {{scripts_path}}/teach.mjs --json
   ```
...
Every render command (`{{command_prefix}}visualize <topic>`, `polish`, `bolder`, `quieter`, `simplify`, `animate`) reads the YAML frontmatter + sidecar at render time and inlines the CSS-form tokens into the artifact's `<style>` element.
